...
from aquordion.blocks.primitive import ProjectorBlock
from aquordion.blocks.utils import chain, kron, uuid_to_expression
from aquordion.parameters import evaluate, stringify

# from aquordion.states import product_state
from aquordion.types import PI, Endianness, TensorType, TNumber
...
def _parametric_matrix(gate: ParametricBlock, values: dict[str, torch.Tensor]) -> torch.Tensor:
    from aquordion.operations import PHASE, RX, RY, RZ, U
...
def _controlled_block_with_params(
    ...
    from aquordion.utils import P1
...
def _swap_block(block: AbstractBlock) -> AbstractBlock:
    ...
    """
    from aquordion.operations import CNOT
...
def _cswap_block(block: AbstractBlock) -> torch.Tensor:
    from aquordion.operations import Toffoli
...
def block_to_tensor(
    ...
    from aquordion import hea, hamiltonian_factory, Z, block_to_tensor
    ...
    ```
    """
    from aquordion.blocks import embedding
...
def _block_to_tensor_embedded(
    ...
    from aquordion.blocks import MatrixBlock
    from aquordion.operations import CSWAP, SWAP, HamEvo
