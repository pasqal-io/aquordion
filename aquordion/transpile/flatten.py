...
def _flat_blocks(block: AbstractBlock, T: Type) -> Generator:
    ...
    from aquordion.transpile.block import _flat_blocks
    from aquordion.blocks import ChainBlock
    from aquordion import chain, X
...
def _(block: AbstractBlock, types: list = [ChainBlock, KronBlock, AddBlock]) -> AbstractBlock:
    ...
    Example:
    ```python exec="on" source="material-block" result="json"
    from aquordion import chain, kron, X
    from aquordion.transpile import flatten
    from aquordion.blocks import ChainBlock, KronBlock, AddBlock
