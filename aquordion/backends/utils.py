...
from aquordion.types import Endianness, ParamDictType
from aquordion.utils import int_to_basis, is_aquordion_shape
...
def validate_state(state: Tensor, n_qubits: int) -> None:
    """Check if a custom initial state conforms to the aquordion or the pyqtorch format."""
    ...
    elif not is_aquordion_shape(state, n_qubits) and not is_pyq_shape(state, n_qubits):
        raise ValueError(
            f"Allowed formats for custom initial state are:\
                  (1) aquordion shape: (batch_size, 2**n_qubits)\
