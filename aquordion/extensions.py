...
from aquordion.backend import Backend, BackendConfiguration
from aquordion.blocks.abstract import TAbstractBlock
from aquordion.engines.differentiable_backend import DifferentiableBackend
from aquordion.types import BackendName, DiffMode, Engine
...
def import_config(backend_name: str | BackendName) -> BackendConfiguration:
    module_path = f"aquordion.backends.{backend_name}.config"
...
def import_backend(backend_name: str | BackendName) -> Backend:
    module_path = f"aquordion.backends.{backend_name}.backend"
    backend: Backend
    try:
        module = importlib.import_module(module_path)
    except (ModuleNotFoundError, ImportError) as e:
        # If backend is not in aquordion, search in extensions.
        module_path = f"aquordion_extensions.backends.{backend_name}.backend"
        try:
            module = importlib.import_module(module_path)
        except (ModuleNotFoundError, ImportError) as e:
...
def _available_backends() -> dict[BackendName, Backend]:
    """Return a dictionary of currently installed, native aquordion backends."""
...
def import_engine(engine_name: str | Engine) -> DifferentiableBackend:
    module_path = f"aquordion.engines.{engine_name}.differentiable_backend"
...
def _available_engines() -> dict[Engine, DifferentiableBackend]:
    """Return a dictionary of currently installed, native aquordion engines."""
...
def _supported_gates(backend_name: str) -> list[TAbstractBlock]:
    ...
    from aquordion import operations

    backend_name = BackendName(backend_name)  # Validate backend name.
    module_path = f"aquordion.backends.{backend_name}"
...
def _gpsr_fns() -> dict:
    """Fallback function for native aquordion GPSR functions if extensions is not present."""
    # avoid circular import
    from aquordion.backends.gpsr import general_psr

    return {DiffMode.GPSR: general_psr}


def _validate_diff_mode(backend: Backend, diff_mode: DiffMode) -> None:
    """Fallback function for native aquordion diff_mode if extensions is not present."""
...
def _set_backend_config(backend: Backend, diff_mode: DiffMode) -> None:
    """Fallback function for native aquordion backends if extensions is not present.
...
# if proprietary aquordion_plus is available import the
# right function since more backends are supported
try:
    module = importlib.import_module("aquordion_extensions.extensions")
