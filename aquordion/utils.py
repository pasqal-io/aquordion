...
if TYPE_CHECKING:
    from aquordion.operations import Projector
...
def basis_to_int(basis: str, endianness: Endianness = Endianness.BIG) -> int:
    ...
    from aquordion.utils import basis_to_int, Endianness
...
def int_to_basis(
    ...
    Examples:
    ```python exec="on" source="material-block" result="json"
    from aquordion.utils import int_to_basis, Endianness
...
def nqubits_to_basis(
    ...
    Examples:
    ```python exec="on" source="material-block" result="json"
    from aquordion.utils import nqubits_to_basis, Endianness, ResultType
...
def print_sympy_expr(expr: sympy.Expr, num_digits: int = 3) -> str:
    ...
    """
    from aquordion.parameters import sympy_to_numeric
...
def is_aquordion_shape(state: ArrayLike, n_qubits: int) -> bool:
...
def validate_values_and_state(
    ...
    if state is not None:
        from aquordion.backends.utils import infer_batchsize
        ...
        if isinstance(state, Tensor):
            ...
            if not is_aquordion_shape(state, n_qubits) or state.shape[0] > 1:
