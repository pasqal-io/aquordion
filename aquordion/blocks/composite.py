...
from aquordion.qubit_support import QubitSupport, QubitSupportType
...
class CompositeBlock(AbstractBlock):
    ...
    Composite blocks are constructed via [`chain`][aquordion.blocks.utils.chain],
    [`kron`][aquordion.blocks.utils.kron], and [`add`][aquordion.blocks.utils.add].
    """
    ...
    def qubit_support(self) -> Tuple[int, ...]:
        from aquordion.blocks.analog import AnalogBlock
    ...
    def _from_dict(cls, d: dict) -> CompositeBlock:
        from aquordion import blocks as aquordionblocks
        from aquordion import operations
        from aquordion.blocks.utils import _construct, tag

        blocks = [
            (
                getattr(operations, b["type"])._from_dict(b)
                if hasattr(operations, b["type"])
                else getattr(aquordionblocks, b["type"])._from_dict(b)
            )
            for b in d["blocks"]
        ]
...
class ChainBlock(CompositeBlock):
    ...
    Constructed via [`chain`][aquordion.blocks.utils.chain]
    """
...
class KronBlock(CompositeBlock):
    ...
    Constructed via [`kron`][aquordion.blocks.utils.kron].
    """
...
class AddBlock(CompositeBlock):
    ...
    Constructed via [`add`][aquordion.blocks.utils.add].
    """
