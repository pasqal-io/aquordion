...
from aquordion.types import TParameter
...
class PrimitiveBlock(AbstractBlock):
    ...
    See [`aquordion.operations`](operations.md) for a full list of
    primitive blocks.
    """
    ...
    def __xor__(self, other: int | AbstractBlock) -> AbstractBlock:
        if isinstance(other, int):
            from aquordion.transpile import repeat
...
class ScaleBlock(ParametricBlock):
    ...
    @property
    def _block_title(self) -> str:
        from aquordion.utils import format_parameter
    ...
    def __contains__(self, other: object) -> bool:
        from aquordion.blocks.composite import CompositeBlock
    ...
    def _from_dict(cls, d: dict) -> ScaleBlock:
        ...
        if hasattr(operations, d["block"]["type"]):
            ...
            block = getattr(aquordionblocks, d["block"]["type"])._from_dict(d["block"])
...
class ParametricControlBlock(ParametricBlock):
    ...
    def _from_dict(cls, d: dict) -> ParametricControlBlock:
        from aquordion.serialization import deserialize
